...
public class csMainDbContext : Microsoft.EntityFrameworkCore.DbContext
{
    ...
    public DbSet<csAddress> Addresses { get; set; }
    public DbSet<csComment> Comments { get; set; }
    public DbSet<csUser> Users { get; set; }
    #endregion
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<csAttraction>()
            .HasOne(a => a.csAddress)
            .WithOne(address => address.csAttraction)
            .HasForeignKey<csAddress>(address => address.AttractionId);


        modelBuilder.Entity<csAttraction>()
                .HasMany(a => a.csComments)
                .WithOne(c => c.csAttraction)
                .HasForeignKey(c => c.AttractionId);


        modelBuilder.Entity<csUser>()
                .HasMany(u => u.csComments)
                .WithOne(c => c.csUser)
